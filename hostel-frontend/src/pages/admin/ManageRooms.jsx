...
import RoomFormModal from "./RoomFormModal"; // IMPORTED

import {
  DoorOpen, Trash2, Plus, Search, Users, Lock, Unlock, CalendarClock,
  Building, Pencil, MessageSquare, Filter, ChevronLeft, ChevronRight
} from "lucide-react";
...
const StatCard = ({ icon: Icon, colorClass, value, label }) => (
  <div className="mr-stat-card">
    <div className={`mr-stat-icon ${colorClass}`}><Icon size={24} /></div>
    <div>
...
const ManageRooms = () => {
  ...
  const handleCreateOrUpdate = async (formData) => {
    try {
      ...
      throw e; // Modal needs to know if it failed
    }
  };
  ...
  const filteredRooms = useMemo(() => {
    return rooms.filter((room) => {
      ...
      const matchesSearch = rNum.toLowerCase().includes(searchTerm.toLowerCase().trim());
  ...
  const getAccessBadgeClass = (gender, isPrivate) => {
    if (gender === "BOYS") return isPrivate ? "badge-boy-private" : "badge-boy-shared";
  ...
  return (
    <div className="mr-container">
      {/* Tooltip */}
      {hoveredComment && (
        <div className="mr-tooltip" style={{ top: hoveredComment.y, left: hoveredComment.x }}>
          {hoveredComment.text}
        </div>
      )}
      ...
      <div className="mr-header">
        <div className="mr-title">
          <div className="mr-title-icon"><DoorOpen size={28} /></div>
          <div>Manage Rooms <div className="mr-subtitle">Space & Pricing</div></div>
        </div>
        {!isWarden && (
      ...
      <div className="mr-stats-grid">
        <StatCard icon={Building} colorClass="icon-blue" value={stats.total} label="Total Rooms" />
        <StatCard icon={CalendarClock} colorClass="icon-cyan" value={stats.defaultType} label="Default Rooms" />
        <StatCard icon={CalendarClock} colorClass="icon-purple" value={stats.monthlyType} label="Monthly Rooms" />
        <StatCard icon={Unlock} colorClass="icon-green" value={stats.shared} label="Shared Rooms" />
        <StatCard icon={Lock} colorClass="icon-red" value={stats.private} label="Private Rooms" />
      </div>
      ...
      <div className="mr-toolbar">
        ...
        <div style={{ display: "flex", gap: "10px" }}>
          <select className="mr-filter-select" value={filterHub} onChange={(e) => setFilterHub(e.target.value)}>
            <option value="ALL">All Hubs</option>
            {hubs.map((h) => <option key={h.id} value={h.hubNumber}>{h.hubNumber}</option>)}
          </select>
          <select className="mr-filter-select" value={filterFloor} onChange={(e) => setFilterFloor(e.target.value)}>
            <option value="ALL">All Floors</option>
            {floors
              .filter((f) => filterHub === "ALL" || f.hubNumber === filterHub)
              .map((f) => <option key={f.id} value={f.floorNumber}>{f.floorNumber}</option>)}
          </select>
          <select className="mr-filter-select" value={filterGender} onChange={(e) => setFilterGender(e.target.value)}>
        ...
        <table className="mr-table">
          <thead className="mr-thead">
            <tr>
              ...
              {!isWarden && <th className="mr-th" style={{ textAlign: "right" }}>Actions</th>}
            </tr>
          </thead>
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan="6" style={{ padding: "40px", textAlign: "center", color: "#94a3b8" }}>No rooms found.</td></tr>
            ) : (
              currentRooms.map((r) => (
                <tr key={r.id} className="mr-tr">
                  ...
                  <td className="mr-td">
                    <span className="mr-type-badge"><Users size={14} />{r.roomType ? r.roomType.replace("SHARING_", "") : "2"} Person</span>
                  </td>
                  <td className="mr-td">
                    <span className={`mr-access-badge ${getAccessBadgeClass(r.reservedFor, r.isPrivate)}`}>
                  ...
                  <td className="mr-td">
                    <div className="mr-price-stack">
                      <div className={`mr-price-tag ${r.isPrivate ? "tag-private" : "tag-shared"}`}>
                        {r.isPrivate ? "Full Room Price" : "Per Person Price"}
                      </div>
                      <div className="mr-price-val">Mo: {parseFloat(r.monthlyPrice).toLocaleString("en-US", { minimumFractionDigits: 2 })}</div>
                      {r.reservationPeriod === "DEFAULT" ? (
                        <>
                          <div className="mr-price-val">We: {r.weeklyPrice ? parseFloat(r.weeklyPrice).toLocaleString("en-US", { minimumFractionDigits: 2 }) : "-"}</div>
                          <div className="mr-price-val">Da: {r.dailyPrice ? parseFloat(r.dailyPrice).toLocaleString("en-US", { minimumFractionDigits: 2 }) : "-"}</div>
                        </>
                      ) : (
                        <span style={{ fontSize: "11px", color: "#9ca3af", fontStyle: "italic" }}>* Monthly Only</span>
                      )}
                    </div>
                  </td>
                  <td className="mr-td">
                    {!isWarden && (
                      <div style={{ display: "flex", justifyContent: "flex-end", gap: "8px" }}>
                        <button onClick={() => openEditModal(r)} className="mr-action-btn btn-edit" title="Edit Room"><Pencil size={16} /></button>
                        <button onClick={() => { setRoomToDelete(r.id); setIsDeleteModalOpen(true); }} className="mr-action-btn btn-delete" title="Delete Room"><Trash2 size={16} /></button>
                      </div>
                    )}
                  </td>
                </tr>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "15px 20px", borderTop: "1px solid #e2e8f0" }}>
            ...
            <div style={{ display: "flex", gap: "5px" }}>
              <button
                ...
                className="mr-page-btn" // Add this class to CSS or use inline style from previous files
                style={{
      ...
      {/* Modal - Rendered via separated component */}
      <RoomFormModal
        isOpen={isModalOpen}
